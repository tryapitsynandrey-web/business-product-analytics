        ...
        for _, row in subscriptions_df.iterrows():
            ...
            while current_date < end:
                ...
                transactions.append({
                    ...
                    'payment_method': self.np_rng.choice(['Credit Card', 'Wire Transfer', 'PayPal'])
                })
                transaction_idx += 1
        ...
        for _, row in customers_df.iterrows():
            ...
            for _ in range(num_tickets):
                ...
                tickets.append({
                    ...
                    'date_closed': (open_date + timedelta(days=self.py_rng.randint(1, 10))).date(),
                    'status': 'Closed',
        ...
        for _, row in customers_df.iterrows():
            if self.py_rng.random() < 0.4: # 40% response rate
                ...
                nps.append({
                    ...
                    'score': score,
                    'feedback_category': self.np_rng.choice(['Pricing', 'Features', 'Support', 'Reliability'])
                })
                nps_idx += 1
