...
class ChurnAnalysisEngine:
    ...
    def analyze_churn_by_segment(
        self, subscriptions: pd.DataFrame, customers: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        merged = pd.merge(
            subscriptions, customers[["customer_id", "segment"]], on="customer_id", how="left"
        )
        ...
        active_counts = (
            active.groupby("segment")["customer_id"].nunique().rename("active_customers")
        )
        churned_counts = (
            churned.groupby("segment")["customer_id"].nunique().rename("churned_customers")
        )

        df = pd.concat([active_counts, churned_counts], axis=1).fillna(0).reset_index()

        df["churn_rate"] = df.apply(
            lambda x: self.calculate_churn_rate(x["active_customers"], x["churned_customers"]),
            axis=1,
        )
        df["retention_rate"] = df.apply(
            lambda x: self.calculate_retention_rate(x["active_customers"], x["churned_customers"]),
            axis=1,
        )
    ...
    def analyze_churn_by_plan(self, subscriptions: pd.DataFrame) -> pd.DataFrame:
        ...
        churned_counts = (
            churned.groupby("plan")["customer_id"].nunique().rename("churned_customers")
        )

        res = pd.concat([active_counts, churned_counts], axis=1).fillna(0).reset_index()

        res["churn_rate"] = res.apply(
            lambda x: self.calculate_churn_rate(x["active_customers"], x["churned_customers"]),
            axis=1,
        )
        res["retention_rate"] = res.apply(
            lambda x: self.calculate_retention_rate(x["active_customers"], x["churned_customers"]),
            axis=1,
        )
