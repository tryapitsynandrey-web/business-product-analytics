...
class AnomalyDetectionEngine:
    def detect_percentage_deviation(self, series: pd.Series, baseline_window: int = 3, threshold: float = 0.2) -> List[Dict[str, Any]]:
        results = []
    ...
    def detect_z_score_anomalies(self, series: pd.Series, threshold: float = 2.0) -> List[Dict[str, Any]]:
        results = []
        if len(series) < 3:
            return results
    ...
    def detect_moving_average_anomalies(self, series: pd.Series, window: int = 3, threshold: float = 0.2) -> List[Dict[str, Any]]:
        results = []
    ...
    def summarize_anomalies(self, metric_name: str, series: pd.Series) -> pd.DataFrame:
        if series.empty:
            return pd.DataFrame(columns=['metric', 'period', 'actual_value', 'baseline_value', 'deviation', 'severity', 'method'])
