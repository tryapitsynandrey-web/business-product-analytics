...
class CohortAnalysisEngine:
    ...
    def calculate_cohort_retention(self, customers: pd.DataFrame, subscriptions: pd.DataFrame) -> pd.DataFrame:
        if customers.empty or subscriptions.empty:
            return pd.DataFrame(columns=['cohort_month', 'period_number', 'customers_in_cohort', 'retained_customers', 'retention_rate'])
        ...
        for cohort, group in df.groupby('cohort_month'):
            ...
            for period in range(13):
                eval_period = pd.Period(cohort, freq='M') + period
                ...
                retained = group[(group['start_date'] <= eval_date) & 
                ...
                results.append({
                    ...
                    'retention_rate': float(retained_count / cohort_size) if cohort_size > 0 else 0.0
                })
                
        return pd.DataFrame(results)

    def calculate_cohort_revenue(self, customers: pd.DataFrame, transactions: pd.DataFrame) -> pd.DataFrame:
        if customers.empty or transactions.empty:
            return pd.DataFrame(columns=['cohort_month', 'period_number', 'revenue', 'revenue_per_customer'])
        ...
        def get_period(row):
            diff = (row['transaction_month'].year - pd.Period(row['cohort_month'], freq='M').year) * 12 + \
        ...
        # Merge cohort sizes
        cohort_sizes = cohorts.groupby('cohort_month')['customer_id'].nunique().reset_index(name='customers_in_cohort')
    ...
    def build_cohort_summary(self, customers: pd.DataFrame, subscriptions: pd.DataFrame, transactions: pd.DataFrame) -> pd.DataFrame:
        if customers.empty:
            return pd.DataFrame(columns=['cohort_month', 'period_number', 'customers_in_cohort', 'retained_customers', 'retention_rate', 'revenue', 'revenue_per_customer'])
        ...
        summary['customers_in_cohort'] = summary['customers_in_cohort'].fillna(summary.groupby('cohort_month')['customers_in_cohort'].transform('max'))
