...
class CohortDiagnosticsEngine:
    def calculate_cohort_delta(
        self, cohort_retention: pd.DataFrame, baseline_retention: float
    ) -> pd.DataFrame:
    ...
    def diagnose_cohort_drivers(
        self,
        cohort_summary: pd.DataFrame,
        usage_data: pd.DataFrame | None = None,
        support_data: pd.DataFrame | None = None,
        nps_data: pd.DataFrame | None = None,
    ) -> pd.DataFrame:
    ...
    def build_cohort_diagnostics(
        self, cohort_summary: pd.DataFrame, baseline_retention: float
    ) -> pd.DataFrame:
        if cohort_summary.empty:
            return pd.DataFrame(
                columns=[
