...
class FunnelAnalysisEngine:
    def calculate_funnel_steps(self, customers: pd.DataFrame, product_usage: pd.DataFrame, subscriptions: pd.DataFrame) -> Dict[str, int]:
        if customers.empty:
            return {'Signup': 0, 'Activation': 0, 'Key Action': 0, 'Paid Conversion': 0, 'Month-1 Retention': 0}
        ...
        if 'is_activated' in customers.columns:
            activated.update(customers[customers['is_activated'] == True]['customer_id'].unique())
        if 'activation_date' in customers.columns:
            activated.update(customers[customers['activation_date'].notnull()]['customer_id'].unique())
            
        # Key Actions
        key_action_users = set()
        if not product_usage.empty and 'key_actions' in product_usage.columns:
            key_action_users = set(product_usage[product_usage['key_actions'] > 0]['customer_id'].unique())
        ...
        if not subscriptions.empty and 'signup_date' in customers.columns:
            ...
            retained = merged[merged['end_date'].isnull() | (merged['end_date'] > merged['signup_date'] + pd.Timedelta(days=30))]
        ...
        return {
            ...
            'Paid Conversion': len(signups.intersection(activated).intersection(key_action_users).intersection(paid_users)),
            'Month-1 Retention': len(signups.intersection(activated).intersection(key_action_users).intersection(paid_users).intersection(retained_users))
        }
    ...
    def identify_largest_bottleneck(self, funnel_summary: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'dropoff_rate': float(max_drop['dropoff_rate'])
        }

    def calculate_segment_funnel(self, customers: pd.DataFrame, product_usage: pd.DataFrame, subscriptions: pd.DataFrame) -> pd.DataFrame:
        if customers.empty:
            return pd.DataFrame(columns=['segment', 'step', 'users', 'conversion_rate', 'dropoff_rate', 'previous_step_users'])
        ...
        for segment, group in customers_df.groupby('segment'):
            ...
            group_usage = product_usage[product_usage['customer_id'].isin(group_cust_ids)] if not product_usage.empty else pd.DataFrame()
            group_subs = subscriptions[subscriptions['customer_id'].isin(group_cust_ids)] if not subscriptions.empty else pd.DataFrame()
