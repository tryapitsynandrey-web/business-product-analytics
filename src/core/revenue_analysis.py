...
class RevenueAnalysisEngine:
    def analyze_revenue_by_month(self, transactions: pd.DataFrame) -> pd.DataFrame:
        if transactions.empty:
            return pd.DataFrame(columns=['period', 'gross_revenue', 'refunds', 'failed_payments', 'net_revenue'])
    ...
    def analyze_revenue_by_segment(self, transactions: pd.DataFrame, customers: pd.DataFrame) -> pd.DataFrame:
        if transactions.empty or customers.empty:
            return pd.DataFrame(columns=['segment', 'gross_revenue', 'refunds', 'failed_payments', 'net_revenue'])
            
        merged = pd.merge(transactions, customers[['customer_id', 'segment']], on='customer_id', how='left')
    ...
    def analyze_revenue_by_plan(self, transactions: pd.DataFrame, subscriptions: pd.DataFrame) -> pd.DataFrame:
        if transactions.empty or subscriptions.empty:
            return pd.DataFrame(columns=['plan', 'gross_revenue', 'refunds', 'failed_payments', 'net_revenue'])
            
        merged = pd.merge(transactions, subscriptions[['subscription_id', 'plan']], on='subscription_id', how='left')
        ...
        grouped = merged.groupby('plan').agg({
            'gross_revenue': 'sum',
            'refunds': 'sum',
            'failed_payments': 'sum'
        }).reset_index()
    ...
    def calculate_revenue_movement(self, transactions: pd.DataFrame) -> pd.DataFrame:
        ...
        if monthly.empty:
            return pd.DataFrame(columns=['period', 'net_revenue', 'previous_revenue', 'revenue_growth_rate'])
        ...
        def calc_growth(row):
            if row['previous_revenue'] == 0:
                return 0.0
            return float((row['net_revenue'] - row['previous_revenue']) / abs(row['previous_revenue']))
    ...
    def summarize_revenue_health(self, transactions: pd.DataFrame) -> Dict[str, Any]:
        if transactions.empty:
            return {'gross_revenue': 0.0, 'net_revenue': 0.0, 'refund_rate': 0.0, 'failed_payment_rate': 0.0, 'status': 'Critical'}
