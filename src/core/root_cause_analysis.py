...
class RootCauseAnalysisEngine:
    def analyze_metric_change(self, metric_name: str, current_value: float, previous_value: float, drivers: Dict[str, float]) -> List[Dict[str, Any]]:
        causes = []
        if previous_value == 0:
            return causes
    ...
    def analyze_churn_drivers(self, churn_summary: Dict[str, Any], support_summary: Optional[Dict[str, Any]] = None, usage_summary: Optional[Dict[str, Any]] = None, nps_summary: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        causes = []
    ...
    def analyze_revenue_drop_drivers(self, revenue_summary: Dict[str, Any], refund_summary: Optional[Dict[str, Any]] = None, failed_payment_summary: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        causes = []
    ...
    def rank_root_causes(self, causes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        def sort_key(c):
            return (sev_map.get(c.get("severity", "Low"), 1), conf_map.get(c.get("confidence", "Low"), 1))
            
        return sorted(causes, key=sort_key, reverse=True)

    def build_root_cause_summary(self, causes: List[Dict[str, Any]]) -> pd.DataFrame:
        if not causes:
            return pd.DataFrame(columns=["cause", "evidence", "severity", "confidence", "recommended_investigation"])
