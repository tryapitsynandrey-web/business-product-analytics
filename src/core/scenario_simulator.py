...
class ScenarioSimulator:
    def simulate_churn_reduction(
        self, baseline_revenue: float, churn_rate: float, improvement_rate: float
    ) -> Dict[str, Any]:
    ...
    def simulate_activation_increase(
        self, signups: int, activation_rate: float, improvement_rate: float, arpu: float
    ) -> Dict[str, Any]:
    ...
    def simulate_arpu_increase(
        self, active_customers: int, current_arpu: float, increase_rate: float
    ) -> Dict[str, Any]:
    ...
    def simulate_failed_payment_recovery(
        self, failed_payment_amount: float, recovery_rate: float
    ) -> Dict[str, Any]:
    ...
    def simulate_margin_improvement(
        self, revenue: float, current_margin: float, improvement_points: float
    ) -> Dict[str, Any]:
    ...
    def run_default_scenarios(self, inputs: Dict[str, float]) -> pd.DataFrame:
        ...
        if all(k in inputs for k in ["baseline_revenue", "churn_rate"]):
            results.append(
                self.simulate_churn_reduction(
                    inputs["baseline_revenue"], inputs["churn_rate"], 0.10
                )
            )

        if all(k in inputs for k in ["signups", "activation_rate", "arpu"]):
            results.append(
                self.simulate_activation_increase(
                    int(inputs["signups"]), inputs["activation_rate"], 0.10, inputs["arpu"]
                )
            )

        if all(k in inputs for k in ["active_customers", "current_arpu"]):
            results.append(
                self.simulate_arpu_increase(
                    int(inputs["active_customers"]), inputs["current_arpu"], 0.05
                )
            )

        if "failed_payment_amount" in inputs:
            results.append(
                self.simulate_failed_payment_recovery(inputs["failed_payment_amount"], 0.30)
            )

        if all(k in inputs for k in ["revenue", "current_margin"]):
            results.append(
                self.simulate_margin_improvement(inputs["revenue"], inputs["current_margin"], 0.02)
            )
