...
CUSTOMER_360_COLUMNS = ["customer_id", "segment", "is_active", "signup_date", "country", "acquisition_channel", "plan", "subscription_status", "current_mrr", "total_revenue", "latest_usage_frequency", "usage_trend", "latest_nps", "support_ticket_count", "failed_payment_count", "churn_risk_score", "churn_risk_band", "revenue_at_risk", "main_driver", "recommended_action", "health_status", "revenue_segment", "usage_segment", "risk_segment", "nps_segment"]
DATA_QUALITY_SCORE_COLUMNS = ["dataset", "completeness_score", "uniqueness_score", "validity_score", "referential_integrity_score", "overall_score", "status", "business_risk"]
HEALTH_SCORE_COLUMNS = ["area", "score", "status", "explanation"]
INTERVENTION_PLAN_COLUMNS = ["intervention_id", "recommendation_title", "category", "target_segment", "affected_customers", "estimated_revenue_impact", "expected_action", "suggested_owner", "confidence_level", "effort_level", "priority_score", "priority_band"]
CHURN_RISK_PROFILE_COLUMNS = ["customer_id", "risk_score", "risk_band", "drivers", "revenue_at_risk", "explanation"]
DECISION_TRACE_COLUMNS = ["trace_id", "created_at", "entity_type", "entity_id", "signal", "triggered_rule", "evidence", "business_impact", "generated_action", "confidence_level"]
METRIC_LINEAGE_COLUMNS = ["metric_name", "display_name", "category", "source_datasets", "required_columns", "formula_description", "business_owner", "business_purpose", "risk_if_misread", "lineage_status"]
RECOMMENDATION_COLUMNS = ["rule_id", "recommendation_title", "category", "customer_id", "segment", "reason", "affected_customers", "estimated_revenue_impact", "impact_score", "confidence_level", "confidence_weight", "suggested_owner", "evidence_source", "priority_score", "priority_band"]
REVENUE_LEAKAGE_COLUMNS = ["leakage_type", "customer_id", "estimated_revenue_loss", "estimated_loss", "affected_customers", "recommended_action", "evidence"]
UI_METRIC_CATALOG_COLUMNS = ["metric_name", "display_name", "category", "business_owner", "business_purpose", "interpretation_notes", "risk_if_misread", "enabled", "implementation_status"]
