...
def test_metric_registry_builds_entries(governance):
    ...
    assert registry.mapped_count + registry.unmapped_count + registry.disabled_count == len(registry.entries)
...
def test_ui_metric_catalog_schema(governance):
    ...
    first = catalog[0]
    expected_keys = {
        "metric_name", "display_name", "category", "business_owner",
        "business_purpose", "interpretation_notes", "risk_if_misread",
        "enabled", "implementation_status"
    }
