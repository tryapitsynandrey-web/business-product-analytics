...
def test_churn_driver_detection():
    engine = RootCauseAnalysisEngine()
    causes = engine.analyze_churn_drivers({"churn_rate": 0.15}, support_summary={"support_burden": 3.0})
    assert len(causes) == 1
    assert causes[0]["cause"] == "High support burden"

def test_revenue_drop_driver_detection():
    engine = RootCauseAnalysisEngine()
    causes = engine.analyze_revenue_drop_drivers({"revenue_growth_rate": -0.15}, refund_summary={"refund_rate": 0.10})
    assert len(causes) == 1
