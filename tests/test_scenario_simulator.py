...
def test_churn_reduction_impact():
    ...
    res = simulator.simulate_churn_reduction(baseline_revenue=100000, churn_rate=0.05, improvement_rate=0.10)
...
def test_activation_increase_impact():
    simulator = ScenarioSimulator()
    res = simulator.simulate_activation_increase(signups=1000, activation_rate=0.20, improvement_rate=0.10, arpu=50)
...
def test_arpu_increase_impact():
    simulator = ScenarioSimulator()
    res = simulator.simulate_arpu_increase(active_customers=1000, current_arpu=50, increase_rate=0.10)
...
def test_failed_payment_recovery():
    simulator = ScenarioSimulator()
    res = simulator.simulate_failed_payment_recovery(failed_payment_amount=10000, recovery_rate=0.30)
    assert res['simulated_value'] == 0.30
    assert res['monthly_impact'] == 3000.0

def test_margin_improvement():
    simulator = ScenarioSimulator()
    res = simulator.simulate_margin_improvement(revenue=100000, current_margin=0.70, improvement_points=0.05)
